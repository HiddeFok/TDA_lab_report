...
plt.savefig("figures/MNINST_example.svg", format="svg")

fig = plt.figure()
fig.set_size_inches((4, 3))
plt.title("Original picture")
plt.imshow(X_copy[4], cmap="gray")
plt.savefig("figures/MNINST_example_original.svg", format="svg")
...
spread = 0.2
PI_vectors = np.zeros((5, 8 * quality * quality))
for i in tqdm([4]):
    bin_vec = X_bin[i]
    hf_vec = X_hf[i]
    df_vec = X_df[i]
