...
for i, weight in enumerate(weightings):
        axs[i].bar(x - width, acc_scores_H0_ada[weight], width, label="H0")
        axs[i].bar(x, acc_scores_H1_ada[weight], width, label="H1")
        axs[i].bar(x + width, acc_scores_con_ada[weight], width, label="H0 & H1")
        axs[i].set_ylabel('Accuracy')
        axs[i].set_xticks(x)
        axs[i].set_xticklabels(kernels)
        axs[i].set_yticks(np.arange(0, 1.1, 0.1))
        axs[i].legend()
        axs[i].set_ylim((0, 1))
        axs[i].set_title("Accuracy of classification with AdaBoost and weighting {}".format(weight))

plt.savefig("figures/accuracies_dynamic_ada.png")
...
x = np.arange(4)
for i, weight in enumerate(weightings):
        axs[i].bar(x - width, acc_scores_H0_grad[weight], width, label="H0")
        axs[i].bar(x, acc_scores_H1_grad[weight], width, label="H1")
        axs[i].bar(x + width, acc_scores_con_grad[weight], width, label="H0 & H1")
        axs[i].set_ylabel('Accuracy')
        axs[i].set_xticks(x)
        axs[i].set_xticklabels(kernels)
        axs[i].set_yticks(np.arange(0, 1.1, 0.1))
        axs[i].legend()
        axs[i].set_ylim((0, 1))
        axs[i].set_title("Accuracy of classification with Gradient Boosting and weighting {}".format(weight))

plt.savefig("figures/accuracies_dynamic_grad.png")
